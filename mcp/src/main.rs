...
impl MemoryServer {
    ...
    #[tool(description = "Save a memory. LLM MUST pre-process before calling: (1) Extract clean single-sentence fact, (2) Determine 2-5 topics from conversation context, (3) Decide scope using this decision tree: 'global'=universal preference, 'lang'=language-specific (needs lang param), 'feature'=product feature across repos (needs feature param), 'repo'=repository-specific (needs repo param), 'module'=directory-specific (needs repo+module params). Choose most specific scope that applies. Server validates scope, checks for duplicates (reinforces if found), detects contradictions (supersedes if found), and stores. Parameters: content (required - pre-processed fact), user_id (required), topics (optional - defaults to empty array), scope (REQUIRED - one of: global/lang/feature/repo/module), repo (optional - required for repo/module scopes), lang (optional - required for lang scope), module (optional - directory path like 'src/auth', required for module scope), feature (optional - product area like 'invoicing', required for feature scope). Call proactively when patterns emerge or user shows preference.")]
